...
func _physics_process(delta):
	...
	for b in physics_bones:
		...
		var local_target_transform: Transform3D = target_skeleton.transform * target_skeleton.get_bone_pose(b.get_bone_id())
		var current_transform: Transform3D = global_transform * get_bone_global_pose(b.get_bone_id())
		var local_current_transform: Transform3D = transform * get_bone_pose(b.get_bone_id())
		var relative_position_difference: Vector3 = (target_skeleton.transform * target_skeleton.get_bone_pose(b.get_bone_id())).origin - (transform * get_bone_pose(b.get_bone_id())).origin
		...
		var relative_rotation_difference: Basis = (local_target_transform.basis * local_current_transform.basis.inverse())
		...
		if relative_rotation_difference.get_euler().length()>.1:
			character_controller.velocity-=position_difference*delta*150
