...
func item_interact():
	GameManager.weatherManager.get_node("UI").visible = false
	...
	GameManager.weatherManager.get_node("UI").visible = true
...
#		console_output = "This operating system is currently under construction.\nThere are no available commands.\nGoodbye.\n--\n>"
		...
		weather_output += "Planetary time is "+str(6+floor(GameManager.weatherManager.simulated_time_of_day/60)).pad_zeros(2)+":"+str(floor(fmod(GameManager.weatherManager.simulated_time_of_day,60))).pad_zeros(2)+"\n"
		weather_output += "Temperature of "+str(GameManager.playerStats.experienced_temperature).pad_decimals(0)+" degrees farenheit.\n"
		weather_output += "Atmospheric pressure of "+str(GameManager.playerStats.experienced_pressure).pad_decimals(0)+" PSI.\n"
		weather_output += str(GameManager.playerStats.experienced_humidity).pad_decimals(0)+" percent humidity.\n--\n>"
