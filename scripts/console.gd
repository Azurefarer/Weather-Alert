...
func item_interact():
	WeatherManager.weather_ui.get_node("UI").visible = false
	...
	WeatherManager.weather_ui.get_node("UI").visible = true
...
#		console_output = "This operating system is currently under construction.\nThere are no available commands.\nGoodbye.\n--\n>"
		...
		weather_output += "Planetary time is "+str(6+floor(WeatherManager.simulated_time_of_day/60)).pad_zeros(2)+":"+str(floor(fmod(WeatherManager.simulated_time_of_day,60))).pad_zeros(2)+"\n"
