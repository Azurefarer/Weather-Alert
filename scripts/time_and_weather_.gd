...
func _ready():
	WeatherManager.weather_ui = self
	while GameManager.player_stats == null:
		await get_tree().physics_frame
	WeatherManager.update()
	if is_multiplayer_authority():
		WeatherManager.initialize_day()
		igmPERrs = float(SIMULATED_DAY_LENGTH)/float(DAY_LENGTH) # 1.19mins/1s
...
#func update():
	#for w in GameManager.player_stats.weather_modifiers:
		#await get_tree().physics_frame
	#WeatherManager.world_temperature = general_hotness*.75+heat_variation*sin(time_of_day/10)*20+30*sin(time_of_day*PI/720)
	#await get_tree().physics_frame
	#WeatherManager.world_pressure = general_pressure*.95+pressure_variation*sin(time_of_day/10)
	#await get_tree().physics_frame
	#WeatherManager.world_humidity = general_humidity*.85+humidity_variation*sin(time_of_day/10)*20
	#await get_tree().physics_frame
	#if is_multiplayer_authority():
		#$UI/Temperature.text = str(GameManager.player_stats.experienced_temperature).pad_decimals(0)+" F"
		#$UI/Pressure.text = str(GameManager.player_stats.experienced_pressure).pad_decimals(0)+" PSI"
